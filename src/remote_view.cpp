...
void Spectator::send_player_info(Player *player)
{
    std::string buffer;
    buffer.apend("P");
    buffer.append(std::to_string(player->get_id()))
    buffer.append(" L");
    buffer.append(std::to_string(player->get_lives()));
    buffer.append(" S");
    buffer.append(std::to_string(player->get_score()));
    buffer.append(" B");
    buffer.append(std::to_string(player->get_bomb_count()));
    buffer.append(" \n");
#ifdef LOG_FILE
    this->out_fs.write(buffer.c_str(), buffer.size());
#endif
